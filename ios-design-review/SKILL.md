...
version: 1.0.0
description: |
  Gerçek donanımda iOS uygulamaları için görsel tasarım denetimi. /ios-qa ile
  aynı StateServer üzerinden gerçek bir iPhone'a bağlanır, her ekranın
  ekran görüntüsünü alır, Apple HIG, DESIGN.md ve tasarım en iyi uygulamalarına
  göre değerlendirir. Her boyutu 0-10 puanlar ve "10 yapmak için ne gerekli"
  çerçevesini kullanır — tarayıcı için /plan-design-review'yu yansıtır.
  Plan aşaması tasarım incelemesi (uygulama öncesi) için /plan-design-review
  kullanın. Canlı web görsel denetimleri için /design-review kullanın.
  "iOS tasarımını incele", "iPhone uygulamasının görsellerini denetle" veya
  "iOS uygulamasının tasarım QA'sı" sorulduğunda kullanın. (gstack)
  Ses tetikleyicileri (konuşmadan metne takma adları): "review the iOS design", "audit the iPhone app visuals", "design QA the iPhone app".
allowed-tools:
  - Bash
  - Read
  - Glob
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"ios-design-review","question_id":"<id>","question_summary":"<kısa>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
# iOS Tasarım İncelemesi

Gerçek bir iOS cihazında tasarımcı gözüyle QA. Görsel tutarsızlıkları, boşluk sorunlarını, hiyerarşi problemlerini, AI-slop kalıplarını ve erişilebilirlik boşluklarını bulur. Her boyutu 0-10 puanlar. iOS deyimlerine uyarlanmış `/plan-design-review`'nun puanlama rubrikasını yansıtır.

## Bağlantı

Çalışan `gstack-ios-qa-daemon`'u kullanır. Çalışan daemon yoksa, `/ios-qa` ile aynı akışla bir tane başlatın (Faz 0-2). Varsayılan olarak salt okunur — değiştirici çağrı yok.

## Boyutlar + puanlama

Uygulamadaki her ekran için 0-10 puanlayın ve 10'a itmek için ne gerekli olduğunu açıklayın:

1. **Tipografi hiyerarşisi.** Display vs body vs caption boyutları Apple HIG ile tutarlı. SF Pro doğru dinamik tip ölçeğinde. Satır yüksekliği font boyutuyla eşleşiyor. Hiçbir yerde 12pt body yok.
2. **Boşluk ritmi.** 4pt veya 8pt grid tutarlı kullanılmış. Sihirli 17/23/31pt dolgular yok. Güvenli alan kenar boşluklarına saygı gösteriliyor.
3. **Renk hiyerarşisi.** Birincil eylem en yüksek kontrast; ikincil soluk; yıkıcı belirgin. Karanlık mod doğru oluşturuluyor. Kontrast oranları gövde metni için WCAG AA (4.5:1) ve büyük metin için (3:1) karşılıyor.
4. **Dokunma hedefleri.** Her etkileşimli öğe >= 44x44pt. 24pt'ten küçük "dokunulabilir metin" yok.
5. **Yükleme + boş + hata durumları.** Her biri mevcut ve kasıtlı. Zaman uyumsuz iş sırasında boş ekranlar yok. Boş durumlar ne yapılacağını açıklıyor.
6. **Erişilebilirlik.** Her etkileşimli öğede VoiceOver etiketleri. Dynamic Type XXL sınırında düzenleri bozmuyor. Reduce Motion'a saygı duyuluyor. Renk körlüğü paleti test edilmiş (deuteranopia en yaygın).
7. **Animasyon disiplini.** Aynı anda 2'den fazla animasyon yok. UI geri bildirimi için süre 200-300ms. Spring sönümleme doğru (ciddi akışlarda sıçramıyor).
8. **iOS deyim uyumu.** Uygun yerlerde native bileşenler kullanıyor (`NavigationStack`, `List`, `Form`, sistem sayfaları). Yeniden icat edilmiş gezinme yok. Telefonda web tarzı hamburger menü yok.
9. **Bilgi yoğunluğu.** Ekran başına içerik yatay kaydırma olmadan sığıyor. Uzun ekranlarda bölüm bağlantıları var. Listeler gerçek iOS liste kalıpları kullanıyor (kaydırmalı silme, bağlamsal menüler).
10. **AI-slop kontrolü.** Genel stok düzenler, içinde kalan "lorem ipsum" verileri, Android'den aktarılan cargo-cult Material Design, AI tarafından oluşturulmuş gibi kokan gradyanlar.

## Döngü

1. `observe` yeteneği ile `POST /session/acquire` (salt okunur).
2. Her ana ekran için (kullanıcının sağladığı ekran listesinden veya erişilebilirlik ağacı aracılığıyla otomatik keşfedilen):
   - `GET /screenshot`
   - `GET /elements`
   - 10 boyutlu rubriki uygula.
   - Bulguları kaydet.
3. Ekran görüntüleri, ekran başına puanlar ve her boyut için "en büyük etki düzeltmesi" önerisi içeren bir markdown raporu oluştur.
4. 7'den düşük her puan için AskUserQuestion kullanın — sorunu önerilen düzeltme + ödünleşim ile sunun, böylece kullanıcı düzeltip düzeltmeyeceğine karar verebilir.

## Çıktı

`~/.gstack/projects/<slug>/ios-design-review-<date>.md` konumuna bir markdown raporu yazın. Ekran görüntülerini satır içi olarak dahil edin. CEO/mühendislik inceleme skill'leri UI değişiklikleri planlarken bu rapora başvurabilir.

## Hata modları

| Belirti | Eylem |
|---|---|
| /screenshot'tan `403 capability_insufficient` | Daemon tailnet modunda ve token `observe` seviyesinin altında — sahip `--capability observe` ile yeni bir token oluşturmalı |
| Ekran görüntüsü siyah/boş | Uygulama ön planda olabilir ancak oluşturmuyor; AskUserQuestion ile uygulamanın beklenen durumda olduğunu onaylayın |
| 10 ekran var, ancak ground-truth ekran listesi 12 demiş | AskUserQuestion: henüz tetiklemediğimiz bir durum arkasına 2 ekran mı gizli? |