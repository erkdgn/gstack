...
version: 1.0.0
description: |
  Bir iOS uygulamasından DebugBridge SPM paketini ve tüm #if DEBUG bağlantılarını
  kaldırır. /ios-qa tarafından kurulan StateServer, DebugOverlay, accessor codegen
  çıktısı ve uygulama taraflı hook'ları temizler. Bu kolaylık akışının bir
  sarmalayıcısıdır — yapısal Release-build koruması (Package.swift koşullu + CI
  swift build -c release kontrolü) güvenlik açısından kritik yoldur.
  "iOS debug bridge'i temizle", "DebugBridge kaldır" veya
  "gstack iOS izlemeyi ayıkla" sorulduğunda kullanın. (gstack)
  Ses tetikleyicileri (konuşmadan metne takma adları): "clean the iOS debug bridge", "remove DebugBridge", "strip the gstack iOS instrumentation".
allowed-tools:
  - Bash
  - Read
  - Edit
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"ios-clean","question_id":"<id>","question_summary":"<kısa>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
# Bir iOS Uygulamasından DebugBridge'ı Ayıkla

Bu skill bir **kolaylık akışıdır**, bir güvenlik mekanizması değildir. Release'te DebugBridge göndermeye karşı yapısal koruma `Package.swift.template`'te (`.when(configuration: .debug)`) artı DebugBridge sembolünün yokluğunu iddia eden CI invariant testindedir. İkisi de `/ios-qa`'nın şablon kurulumunun bir parçası olarak gönderilir.

Bu skill şu geliştiriciler için vardır:

- DebugBridge dosyalarını manuel olarak kopyalamış olanlar (`/ios-qa`'nın SPM kurulumunu kullanmadan).
- Güvenlik denetiminden önce yönlendirilmiş, geri alınabilir bir kaldırma akışı isteyenler.
- gstack'ten uzaklaşmak isteyen ve temiz bir çıkış isteyenler.

## Ne kaldırır

Her öğe yalnızca AskUserQuestion onayından sonra geri alınır:

1. `Package.swift`'ten `DebugBridge` SPM hedefi.
2. Uygulamanın `@main` girişinde `DebugBridgeManager.shared.start()` çağıran `#if DEBUG` bloğu.
3. Kanonik uygulama durum yapısındaki `@Snapshotable` özellik sarmalayıcıları (codegen algılama işaretleyicileri — sarmalayıcı dosyası DebugBridge içinde yaşar, bu nedenle SPM bağımlılığını kaldırmak sarmalayıcıyı da kaldırır).
4. Uygulama kaynağında herhangi bir yerde oluşturulan `StateAccessor.swift` dosyaları.
5. Cihazda `NSTemporaryDirectory()` altındaki `gstack-ios-qa.token` dosyası (en iyi çaba — yalnızca /ios-clean çalıştığında cihaz bağlıysa çalışır).

## Ne dokunmaz

- Uygulama iş mantığı, view model'leri, view kodu.
- `#if DEBUG` blokları dışındaki her şey.
- Diğer test veya QA altyapısı.

## Faz 1: Envanter

1. Uygulama kaynağında `import DebugBridge` glob arayın.
2. `#if DEBUG ... DebugBridgeManager` bloklarını glob arayın.
3. `StateAccessor.swift` dosyalarında `// Auto-generated state accessor` başlıklarını glob arayın.
4. `Package.swift`'te DebugBridge bağımlılık girişi için ayrıştırın.
5. Kullanıcıya neyin kaldırılacağını gösterin (dosya listesi + satır sayıları).
   AskUserQuestion: devam et, kuru çalıştırma veya iptal.

## Faz 2: Kaldır

Kullanıcının onayladığı her öğe için:

1. Import + `#if DEBUG` bloğunu ayıklamak için Edit aracını kullanın (çevreleyen kodu bozulmaz tutun).
2. `Package.swift`'ten `.package(url:...DebugBridge...)` girişini ve `"DebugBridge"` referansı olan `targets`'ları kaldırmak için Edit aracını kullanın.
3. Oluşturulan `StateAccessor.swift` dosyalarını silin.
4. Bridge olmadan Release'in oluşturduğunu doğrulamak için `xcodebuild -scheme <SchemeName> -destination 'platform=iOS,id=<UDID>' build install -configuration Release` çalıştırın. Eksik bir DebugBridge sembolü üzerinde başarısız olursa, kaldırma eksiktir — DURDURUN ve raporlayın.

## Faz 3: Doğrula

1. `! grep -r "DebugBridge" <app-source-dir>` (eşleşme yok).
2. `! grep -r "@Snapshotable" <app-source-dir>` (eşleşme yok).
3. `swift build -c release` başarılı olur.
4. Oluşturulan ikili üzerinde `nm -j`, DebugBridge sembollerini göstermez.

Temizleme sonucunu + neyin kaldırıldığının tek satırlık bir özetini raporlayın.

## Geri alınabilirlik

Her Edit + silme bir git işlemidir; kullanıcı geri almak için `git restore` kullanabilir.
Bu skill asla force-push yapmaz, asla amend etmez, asla SPM önbelleğini silmez —
bunlar kullanıcı seçimleridir.