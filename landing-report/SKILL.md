...
version: 0.1.0
description: |
  Salt okunur kuyruk panosu. Hangi VERSION yuvalarının açık PR'lar tarafından
  talep edildiğini, hangi kardeş Conductor çalışma alanlarının yakın zamanda
  gönderilecek WIP işlerine sahip olduğunu ve /ship'in sonraki hangi yuvayı
  seçeceğini gösterir. Hiçbir değişiklik yapmaz — yalnızca bir anlık görüntü.
  "landing report", "what's in the queue", "show me open PRs" veya
  "which version do I claim next" sorulduğunda kullanın. (gstack)
triggers:
  - landing report
  - version queue
  - ship queue
...
# /landing-report — Sürüm Kuyruğu Panosu
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"landing-report","question_id":"<id>","question_summary":"<kısa>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
5-10 paralel Conductor çalışma alanı çalıştırırken, bir bakışta hangi sürüm numaralarının kim tarafından talep edildiğini ve bir sonraki `/ship`'inizin hangi yuvaya ineceğini görmek yardımcı olur. Bu skill, `/ship`'in kullandığı aynı `bin/gstack-next-version` aracına salt okunur bir çağrıdır, ancak hiçbir şey değiştirmez. Bunu VERSION numaraları için `gh pr list` olarak düşünün.

---

## Adım 1: Platformu ve temel dalı algıla

Diğer gstack skill'leriyle aynı algılama.
...
## Adım 3: Kuyruğu sorgula

Araçtan üç kez çağırın — her artırma düzeyi için bir kez — böylece kullanıcı micro/patch/minor/major için ne talep edeceğini görür. Ucuz (aynı gh çağrısı bun tarafından önbelleğe alınır).
...
## Adım 4: Panoyu oluştur

Tek bir tablo çıktısı oluşturun. Kuyruk + kardeşler için `patch` düzeyindeki JSON'ı kanonik olarak kullanın (bunlar artırma düzeyleri arasında aynıdır; yalnızca `.version` farklıdır).

`jq` ile çıkarın:
- `.host` — github | gitlab | unknown
- `.offline` — sorgu başarısız oldu mu?
- `.claimed` — {pr, branch, version, url} dizisi
- `.siblings` — bulunan tüm kardeş worktree'ler
- `.active_siblings` — yakında gönderilecek olasılığı olan alt küme

Bu tam formatta oluşturun:
...
║ Host:    <github|gitlab|unknown>                                 ║
║ Durum:   <ÇEVRİMİÇİ|ÇEVRİMDIŞI: kuyuk-bilinci kullanılamıyor>  ║
╚══════════════════════════════════════════════════════════════════╝

<base> üzerinde sürüm talep eden açık PR'lar:
  #1152  alpha-branch         → v1.7.0.0
  #1153  beta-branch          → v1.7.0.0  ⚠ #1152 ile çarpışma
  #1151  gamma-branch         → v1.6.5.0

Kardeş Conductor worktree'leri (<workspace_root>):
  yol                        dal                 VERSION      son kayıt   PR
  ──────────────────────────────────────────────────────────────────────────────────
  ../tokyo-v2                 feat/dashboard         v1.7.1.0    3 saat önce  none  ★ aktif
  ../melbourne                feat/review            v1.6.0.0    12 gün önce  none
  ../osaka                    feat/payments          v1.8.0.0    5 saat önce  #1155

★ aktif = temelden ileride VERSION'e sahip VE son kayıt < 24 saat VE açık PR yok.
  Bunlar muhtemelen yakında gönderilecek olanlar.

Şimdi /ship çalıştırsaydınız, şu yuvayı talep ederdiniz:
  micro artırma:  v1.6.3.1   (kuyruk-ilerlemesi: yok)
  patch artırma:  v1.7.1.0   (talep edilen 1.7.0.0'ı geçti)
  minor artırma:  v1.8.0.0   (talep edilen 1.7.0.0'ı geçti)
  major artırma:  v2.0.0.0   (büyük çarpışma yok)
```

Çevrimdışı / bilinmeyen host çıktısı için daha kısa bir blok yazdırın:

```
╔══════════════════════════════════════════════════════════════════╗
║                     GSTACK LANDING REPORT                        ║
╠══════════════════════════════════════════════════════════════════╣
║ Durum:   ÇEVRİMDIŞI — kuyruk-bilinci kullanılamıyor             ║
║ Neden:   <uyarılardan çevrimdışı neden>                          ║
╚══════════════════════════════════════════════════════════════════╝

Geri dönüş: yerel VERSION artırmaları hala çalışır, ancak çarpışmalar algılanamaz.
```

---

## Adım 5: Sonraki eylemi öner

Tabloyu oluşturduktan sonra, şunlardan BİRİNİ önerin:

1. **Kuyrukta çarpışmalar varsa** (iki açık PR aynı sürümü talep ediyorsa):
   "⚠ İki açık PR v<X>'de çarpışıyor. İkinci birleşen ya ilk CHANGELOG girişinin üzerine yazar ya da yinelenen bir giriş gönderir. Bir yazarın bir sonraki boş yuvayı seçmek için /ship'i yeniden çalıştırmasını istemeyi düşünün."

2. **Aktif bir kardeş, kullanıcının dal sürümünden daha yüksek bir sürüme sahipse:**
   "Kardeş worktree <path>, <N> saat önce v<X> kaydetmiş ve henüz PR açmamış.
   Bu iş önce gönderilirse, dalınızın birleşme zamanında yeniden artırılması gerekir."

3. **Her şey temiz görünüyorsa:**
   "Kuyruk temiz. Sonraki /ship çarpışma olmadan bir yuvayı talep edecek."

---

## Plan Modu

PLAN MODE EXCEPTION — HER ZAMAN ÇALIŞTIR. Bu skill tamamen salt okunurdur: dosya yazması yok, git değişikliği yok, ağ durumu değişikliği yok. Plan modunda çalıştırmak güvenlidir.