...
version: 1.0.0
description: |
  Daha önce /context-save tarafından kaydedilen çalışma bağlamını geri yükler. En son
  kaydedilen durumu (varsayılan olarak tüm dallar arasında) yükler, böylece kaldığınız
  yerden devam edebilirsiniz — Conductor çalışma alanı aktarımları arasında bile.
  "resume", "restore context", "where was I" veya
  "pick up where I left off" sorulduğunda kullanın. /context-save ile eşleştirin.
  Eskiden /checkpoint resume — mevcut ortamlarda Claude Code /checkpoint'ı
  native bir geri sarma takma adı olarak ele aldığı için yeniden adlandırıldı. (gstack)
allowed-tools:
  - Bash
  - Read
  - Glob
...
- Eksik `~/.claude/skills/gstack/.feature-prompted-continuous-checkpoint`: Sürekli kontrol noktası otomatik kayıtları için AskUserQuestion. Kabul edilirse, `~/.claude/skills/gstack/bin/gstack-config set checkpoint_mode continuous` çalıştırın. Her zaman marker dosyasına dokunun.
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"context-restore","question_id":"<id>","question_summary":"<kısa>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
# /context-restore — Kaydedilen Çalışma Bağlamını Geri Yükle

Bir **Kıdemli Mühendisin bir meslektaşının titiz oturum notlarını okuduğunuz** ve tam olarak kaldıkları yerden devam ettiğiniz bir roldesiniz. Göreviniz, en son kaydedilen bağlamı yüklemek ve kullanıcıyı kesintiye uğratmadan işe devam edebilmesi için net bir şekilde sunmaktır.

**SERTI GEÇİT:** Kod değişiklikleri UYGULAMAYIN. Bu skill yalnızca kaydedilen bağlam dosyalarını okur ve özeti sunar.

**Varsayılan: tüm dallar arasında en son kaydedilen bağlamı yükleyin.** Bu, varsayılan olarak geçerli dala filtreleyen `/context-save list`'ten kasıtlı olarak farklıdır. `/context-restore`, Conductor çalışma alanı aktarımı içindir — bir dalda kaydedilen bağlam diğerinden devam ettirilebilir.

**Aday kümesini geçerli dala göre filtrelemeyin.** `list` akışı bunu yapar; `/context-restore` bunu yapmaz.

---

## Komutu algıla

Kullanıcının girdisini ayrıştırın:

- `/context-restore` → en son kaydedilen bağlamı yükle (herhangi bir dal)
- `/context-restore <başlık-parçası-veya-numara>` → belirli bir kaydedilen bağlamı yükle
- `/context-restore list` → kullanıcıya "`/context-save list` kullanın — listeleme kaydetme tarafında bulunur" deyin ve çıkın. Burada mod algılama yok.

---

## Geri yükleme akışı

### Adım 1: Kaydedilen bağlamları bul
...
if [ ! -d "$CHECKPOINT_DIR" ]; then
  ...
  # ls -1t yerine find + sort kullanın. İki nedenle:
  # 1. Kanonik sıralama, dosya adı YYYYMMDD-HHMMSS önekidir (kopyalar/rsync
  #    arasında kararlıdır). Dosya sistemi mtime değişir ve yetkili değildir.
  # 2. macOS'ta, sonuç sıfır olduğunda `find ... | xargs ls -1t` cwd'yi
  #    listeler. Boş girdide `sort -r` temiz bir şekilde hiçbir şey döndürmez.
  # En son 20 ile sınırlayın: 10k kaydedilen dosyası olan bir kullanıcı bağlam
  #  penceresini yalnızca listelemek için patlatmamalıdır. /context-save list sayfalama yapar.
...
**Adaylar, daldan bağımsız olarak dizindeki her `.md` dosyasını içerir**
(dal frontmatter'da kaydedilir, burada filtreleme için kullanılmaz). Bu, Conductor çalışma alanı aktarımını mümkün kılar.

### Adım 2: Doğru dosyayı yükle

- Kullanıcı bir başlık parçası veya numara belirttiyse: adaylar arasında eşleşen dosyayı bulun.
- Aksi takdirde: yukarıdaki `sort -r` tarafından döndürülen **ilk dosyayı** yükleyin — bu en yeni `YYYYMMDD-HHMMSS` önekidir, yani kanonik "en son" olanıdır.

Seçilen dosyayı okuyun ve bir özet sunun:

```
BAĞLAM GERİ YÜKLENİYOR
════════════════════════════════════════
Başlık:       {title}
Dal:          {frontmatter'dan dal}
Kaydedildi:   {zaman damgası, okunabilir}
Süre:         Son oturum {biçimlendirilmiş süre} idi (varsa)
Durum:        {durum}
════════════════════════════════════════

### Özet
{kaydedilen dosyadan özet}
...
Geçerli dal, kaydedilen bağlamın dalından farklıysa, bunu not edin:
"Bu bağlam `{branch}` dalında kaydedildi. Şu anda `{current branch}` dalındasınız. Devam etmeden önce dal değiştirmek isteyebilirsiniz."

### Adım 3: Sonraki adımları sun

Sunudan sonra, AskUserQuestion ile sorun:

- A) Kalan öğeler üzerinde çalışmaya devam et
- B) Kaydedilen tam dosyayı göster
- C) Yalnızca bağlama ihtiyacım var, teşekkürler

A ise, ilk kalan iş öğesini özetleyin ve oradan başlamayı önerin.

---

## Kaydedilen bağlamlar yoksa

Adım 1 `NO_CHECKPOINTS` yazdıysa, kullanıcıya söyleyin:

"Henüz kaydedilen bağlam yok. Önce geçerli çalışma durumunuzu kaydetmek için `/context-save` çalıştırın, ardından `/context-restore` onu bulacaktır."

---

## Önemli Kurallar

- **Asla kodu değiştirmeyin.** Bu skill yalnızca kaydedilen dosyaları okur ve sunar.
- **Varsayılan olarak her zaman tüm dallar arasında arayın.** Çapraz-dal geri yükleme tüm amacıdır. Yalnızca kullanıcı dalı belirten bir başlık parçası eşleşmesi aracılığıyla açıkça istediğinde dala göre filtreleyin.
- **"En son", dosya adı `YYYYMMDD-HHMMSS` önekidir**, `ls -1t` (dosya sistemi mtime) değil. Dosya adları dosya sistemi işlemleri arasında kararlıdır; mtime değildir.
- **Bu bir gstack skill'idir, Claude Code yerleşik değildir.** Kullanıcı `/context-restore` yazdığında, bu skill'i Skill aracı aracılığıyla çağırın.