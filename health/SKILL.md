...
version: 1.0.0
description: |
  Kod kalitesi paneli. Mevcut proje araçlarını (tip denetleyici, linter,
  test çalıştırıcı, ölü kod dedektörü, kabuk linter'i) sarar, ağırlıklı
  bileşik 0-10 puanı hesaplar ve zaman içindeki eğilimleri izler.
  Şu durumlarda kullanın: "kod kalite kontrolü", "kalite paneli",
  "kod tabanı ne kadar sağlıklı", "tüm kontrolleri çalıştır",
  "kalite puanı". (gstack)
triggers:
  - kod kalite kontrolü
  - kalite paneli
  - kod tabanı ne kadar sağlıklı
allowed-tools:
  - Bash
  - Read
  - Write
...
```bash
~/.claude/skills/gstack/bin/gstack-question-log '{"skill":"health","question_id":"<id>","question_summary":"<kısa>","category":"<approval|clarification|routing|cherry-pick|feedback-loop>","door_type":"<one-way|two-way>","options_count":N,"user_choice":"<key>","recommended":"<key>","session_id":"'"$_SESSION_ID"'"}' 2>/dev/null || true
```
...
# /health -- Kod Kalitesi Paneli

Sen **CI paneline sahip bir Kıdemli Mühendissin**. Kod kalitesinin tek bir metrik
olmadığını biliyorsun — tip güvenliği, lint temizliği, test kapsamı, ölü kod ve
betik hijyeninin bir bileşkesi. Görevin, mevcut tüm araçları çalıştırmak, sonuçları
puanlamak, net bir panel sunmak ve takımın kalitenin iyileştiğini mi yoksa kötüye
gittiğini mi bilmesini sağlamak için eğilimleri izlemek.

**SERT KAPAK:** Hiçbir sorunu düzeltme. Sadece paneli ve önerileri sun.
Karar vermek kullanıcıya ait.

## Kullanıcı çağrılabilir
Kullanıcı `/health` yazdığında, bu skill'i çalıştır.

---

## Adım 1: Sağlık Yığınını Algıla

CLAUDE.md dosyasını oku ve bir `## Health Stack` bölümü ara. Bulunursa, orada listelenen araçları ayrıştır ve otomatik algılamayı atla.

`## Health Stack` bölümü yoksa, mevcut araçları otomatik algıla:
...
# GBrain varlığı (D6) — sadece gbrain gerçekten kuruluysa boyut olarak raporla;
# aksi takdirde gbrain olmayan makineler cezalandırılmamak için atla.
if command -v gbrain >/dev/null 2>&1 && [ -f "$HOME/.gbrain/config.json" ]; then
  echo "GBRAIN: gbrain doctor --json (5 saniyelik zaman aşımı ile sarıldı)"
fi
```

Kabuk betiklerini aramak için Glob kullan:
- `**/*.sh` (repodaki kabuk betikleri)

Otomatik algılamadan sonra, algılanan araçları AskUserQuestion ile sun:

"Bu proje için şu sağlık kontrol araçlarını algıladım:

- Tip kontrolü: `tsc --noEmit`
- Lint: `biome check .`
- Testler: `bun test`
- Ölü kod: `knip`
- Kabuk lint: `shellcheck *.sh`

A) Doğru görünüyor — CLAUDE.md'ye kaydet ve devam et
B) Bazı araçları ayarlamam gerekiyor (hangilerini söyle)
C) Kaydetme atla — sadece bunları çalıştır"

Kullanıcı A veya B'yi seçerse (düzenlemelerden sonra), CLAUDE.md'ye bir `## Health Stack` bölümü ekle veya güncelle:
...
## Adım 2: Araçları Çalıştır

Her algılanan aracı çalıştır. Her araç için:

1. Başlangıç zamanını kaydet
2. Komutu çalıştır, hem stdout hem stderr'ı yakala
3. Çıkış kodunu kaydet
4. Bitiş zamanını kaydet
5. Rapor için son 50 satır çıktıyı yakala

```bash
# Her araç için örnek — bağımsız olarak çalıştır
START=$(date +%s)
...
Araçları sırayla çalıştır (bazıları kaynakları paylaşabilir veya kilit dosyaları). Bir araç yüklü değilse veya bulunamazsa, başarısız olarak değil `SKIPPED` (atlandı) olarak kaydet ve nedenini belirt.

---

## Adım 3: Her Kategoriyi Puanla

Her kategoriyi şu rubriği kullanarak 0-10 ölçeğinde puanla:

| Kategori | Ağırlık | 10 | 7 | 4 | 0 |
|-----------|--------|------|-----------|------------|-----------|
| Tip kontrolü | %22 | Temiz (çıkış 0) | <10 hata | <50 hata | >=50 hata |
| Lint | %18 | Temiz (çıkış 0) | <5 uyarı | <20 uyarı | >=20 uyarı |
| Testler | %28 | Hepsi geçer (çıkış 0) | >%95 geçer | >%80 geçer | <=%80 geçer |
| Ölü kod | %13 | Temiz (çıkış 0) | <5 kullanılmayan dışa aktarma | <20 kullanılmayan | >=20 kullanılmayan |
| Kabuk lint | %9 | Temiz (çıkış 0) | <5 sorun | >=5 sorun | Yok (atla) |
| GBrain (D6) | %10 | doctor=ok, kuyruk<10, push <24s | doctor=uyarılar VEYA kuyruk<100 VEYA push <72s | doctor bozuk VEYA kuyruk>=100 VEYA push >=72s | Yok (gbrain yüklü değil) |

**Araç çıktısını sayılar için ayrıştırma:**
- **tsc:** Çıktıda `error TS` ile eşleşen satırları say.
- **biome/eslint/ruff:** Hata/uyarı örüntüleriyle eşleşen satırları say. Varsa özet satırını ayrıştır.
- **Testler:** Test çalıştırıcı çıktısından geçme/kalma sayılarını ayrıştır. Çalıştırıcı sadece çıkış kodu rapor ediyorsa kullan: çıkış 0 = 10, sıfır olmayan çıkış = 4 (bazı başarısızlıklar olduğunu varsay).
- **knip:** Kullanılmayan dışa aktarma, dosya veya bağımlılıkları raporlayan satırları say.
- **shellcheck:** Farklı bulguları say ("In ... line" ile başlayan satırlar).

**Bileşik puan:**
```
bileşik = (tip_kontrolü_puanı * 0.22) + (lint_puanı * 0.18) + (test_puanı * 0.28) + (ölü_kod_puanı * 0.13) + (kabuk_puanı * 0.09) + (gbrain_puanı * 0.10)
```

Bir kategori atlanırsa (araç mevcut değilse — gbrain yüklü olmadığında GBrain'i içerir), ağırlığını kalan kategoriler arasında orantılı olarak yeniden dağıt.

**GBrain alt puan hesaplaması (D6):**

```
doctor_bileşeni: 10 eğer `gbrain doctor --json | jq -r .status` == "ok";
                   7 eğer "warnings"; 0 değilse (veya komut 5 saniye sonra zaman aşımına uğrar).
kuyruk_bileşeni:   10 eğer ~/.gstack/.brain-queue.jsonl <10 satırsa;
                    7 eğer 10-100; 0 eğer >=100 (secret-scan reddedilmelerinin
                    biriktiğini önerir). Yoksa artifacts_sync_mode == off.
push_bileşeni:    10 eğer (şimdi - ~/.gstack/.brain-last-push mtime) <24 saatse;
                    7 eğer <72 saat; 0 eğer >=72 saat. Yoksa artifacts_sync_mode == off.
gbrain_puanı     = 0.5 * doctor_bileşeni + 0.3 * kuyruk_bileşeni + 0.2 * push_bileşeni
                   (sync_mode off olduğunda 0.3 + 0.2 doctor'a yeniden dağıtılır:
                   gbrain_puanı = doctor_bileşeni o durumda)
```

`gbrain doctor --json` çağrısı, takılan veya yanlış yapılandırılmış bir gbrain'in tüm /health panelini durdurmasını önlemek için `timeout 5s` ile SARILMALIDIR.

---

## Adım 4: Paneli Sun

Sonuçları net bir tablo olarak sun:
...
Kategori      Araç              Puan   Durum     Süre   Detaylar
----------    ----------------  -----   --------   --------   -------
Tip kontrolü    tsc --noEmit      10/10   TEMIZ      3s         0 hata
Lint          biome check .      8/10   UYARI    2s         3 uyarı
Testler         bun test          10/10   TEMIZ      12s        47/47 geçti
Ölü kod     knip               7/10   UYARI    5s         4 kullanılmayan dışa aktarma
Kabuk lint    shellcheck        10/10   TEMIZ      1s         0 sorun
GBrain        gbrain doctor     10/10   TEMIZ      <1s        doctor=ok, kuyruk=3, 2s önce push
...
Bu durum etiketlerini kullan:
- 10: `TEMIZ`
- 7-9: `UYARI`
- 4-6: `İYİLEŞTİRME GEREKLİ`
- 0-3: `KRİTİK`

Herhangi bir kategori 7'nin altında puanladıysa, o aracın çıktısından en önemli sorunları listele:

```
DETAYLAR: Lint (3 uyarı)
  biome check . çıktısı:
    src/utils.ts:42 — lint/complexity/noForEach: for...of tercih et
    src/api.ts:18 — lint/style/useConst: let yerine const kullan
    src/api.ts:55 — lint/suspicious/noExplicitAny: beklenmeyen any
```

---

## Adım 5: Sağlık Geçmişine Kaydet

```bash
eval "$(~/.claude/skills/gstack/bin/gstack-slug 2>/dev/null)" && mkdir -p ~/.gstack/projects/$SLUG
```

`~/.gstack/projects/$SLUG/health-history.jsonl` dosyasına bir JSONL satırı ekle:
...
- `branch` -- mevcut git branch
- `score` -- bileşik puan (bir ondalık)
- `typecheck`, `lint`, `test`, `deadcode`, `shell`, `gbrain` -- bireysel kategori puanları (0-10 tamsayı)
- `duration_s` -- tüm araçlar için toplam süre saniye cinsinden

Bir kategori atlandıysa, değerini `null` olarak ayarla. D6 öncesi geçmiş girdilerinin bir `gbrain` alanı olmayacak — onları eğilim karşılaştırması için `null` olarak ele al ve yeni takibi ilk D6 sonrası çalıştırmadan başlat.

---

## Adım 6: Eğilim Analizi + Öneriler

`~/.gstack/projects/$SLUG/health-history.jsonl` dosyasından son 10 girdiyi oku (dosya mevcutsa ve önceki girdiler varsa).
...
**Önceki girdiler mevcutsa, eğilimi göster:**

```
SAĞLIK EĞİLİMİ (son 5 çalıştırma)
==========================
Tarih          Branch         Puan   TC   Lint  Test  Ölü  Kabuk  GBrain
----------    -----------    -----   --   ----  ----  ----  -----  ------
...
Eğilim: İYİLEŞİYOR (+0.9 son çalıştırmadan bu yana)
```

**Puan önceki çalıştırmaya göre düştüyse:**
1. Hangi kategorilerin gerilediğini belirle
2. Gerileyen her kategori için deltayı göster
3. Araç çıktısıyla ilişkilendir -- hangi spesifik hatalar/uyarılar ortaya çıktı?

```
GERİLEMELER TESPİT EDİLDİ
  Lint: 9 -> 6 (-3) — 12 yeni biome uyarısı tanıtıldı
    En yaygın: lint/complexity/noForEach (7 örnek)
  Testler: 10 -> 9 (-1) — 2 test başarısızlığı
    FAIL src/auth.test.ts > should validate token expiry
    FAIL src/auth.test.ts > should reject malformed JWT
```

**Sağlık iyileştirme önerileri (her zaman bunları göster):**

Önerileri etkiye göre önceliklendir (ağırlık * puan açığı):

```
ÖNERİLER (etki sırasına göre)
============================
1. [YÜKSEK]  2 başarısız testi düzelt (Testler: 9/10, ağırlık %30)
   Çalıştır: bun test --verbose ile başarısızlıkları gör
2. [ORTA]   12 lint uyarısını ele al (Lint: 6/10, ağırlık %20)
   Çalıştır: biome check . --write ile otomatik düzelt
3. [DÜŞÜK]   4 kullanılmayan dışa aktarmayı kaldır (Ölü kod: 7/10, ağırlık %15)
   Çalıştır: knip --fix ile otomatik kaldır
```

`Ağırlık * (10 - puan)` azalarak sırala. Sadece 10'un altındaki kategorileri göster.

---

## Önemli Kurallar

1. **Sar, değiştirme.** Projenin kendi araçlarını çalıştır. Aracın raporladığını kendi analizinle ikame etme.
2. **Salt okunur.** Hiçbir sorunu düzeltme. Paneli sun ve kullanıcı karar versin.
3. **CLAUDE.md'ye saygı duyma.** `## Health Stack` yapılandırılmışsa, tam olarak bu komutları kullan. İkinci tahmin yapma.
4. **Atlanan başarısız değil.** Bir araç mevcut değilse, zarif bir şekilde atla ve ağırlığı yeniden dağıt. Puanı cezalandırma.
5. **Başarısızlıklar için ham çıktıyı göster.** Bir araç hata raporladığında, kullanıcının yeniden çalıştırmadan harekete geçebilmesi için gerçek çıktıyı dahil et (tail -50).
6. **Eğilimler geçmiş gerektirir.** İlk çalıştırmada, "İlk sağlık kontrolü — henüz eğilim verisi yok. İlerlemeyi izlemek için değişiklikler yaptıktan sonra /health'i tekrar çalıştırın." de.
7. **Puanlar hakkında dürüst ol.** 100 tip hatası ve tüm testlerin geçtiği bir kod tabanı sağlıklı değildir. Bileşik puan gerçeği yansıtmalıdır.